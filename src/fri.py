...
class Fri:
    ...
    def commit(self, codeword, proof_stream, round_index=0):
        ...
        for r in range(self.num_rounds()):
            ...
            # split and fold
            # codeword = 2^-1 * ( (1 + alpha / (offset * (omega^i)) ) * codeword[i] + (1 - alpha / (offset * (omega^i)) ) * codeword[len(codeword)//2 + i] )
    ...
    def query(self, current_codeword, next_codeword, c_indices, proof_stream):
        ...
        a_indices = [index for index in c_indices] # A point with index i
        b_indices = [index + len(current_codeword)//2 for index in c_indices] # B point with index N/2+i
    ...
    def sample_indices(self, seed, size, reduced_size, number):
        assert(number <= 2 * reduced_size, "not enough entropy in indices wrt the codeword")
        ...
        while len(indices) < number:
            ...
            counter += 1
            if reduced_size not in reduced_indices:
                indices += [index]
    ...
    def verify( self, proof_stream, polynomial_values ):
        ...
        for r in range(0, self.num_rounds()-1):
            ...
            for s in range(self.num_colinearity_tests):
                ...
                cx = alphas[r]
                if Polynomial.test_colinearity([(ax, ay), (bx, by), (cx, cy)]) == False:
