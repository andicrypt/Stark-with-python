...
class Polynomial:
    ...
    def __add__(self, other):
        ...
        zero = self.coefficients[0].field.zero()
        coeffs = [zero] * max(len(self.coefficients), len(other.coefficients))
    ...
    def divide(numerator, denominator):
        ...
        zero = denominator.coefficients[0].field.zero()
        remainder = Polynomial([n for n in numerator.coefficients])
        quotient_coefficients = [zero for i in range(numerator.degree() - denominator.degree() + 1)]
        ...
        for i in range(numerator.degree() - denominator.degree() + 1):
            ...
            subtractee = Polynomial([zero] * shift + [coefficient]) * denominator
    ...
    def __xor__(self, exponent):
        if self.is_zero():
            return Polynomial([])
        one = self.coefficients[0].field.one()
        if exponent == 0:
